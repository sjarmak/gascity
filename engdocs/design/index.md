...
| `abort-scope-retry-attempt-exemption-ordering` | Implemented | Reorders the retry-attempt exemption in `beadOutcomeFailed` ahead of the `gc.outcome=fail` short-circuit so retries are not inert under `gc.on_fail=abort_scope`; converges the three abort-scope decision sites on one predicate |
