...
| `post-close-scope-failure-attestation-v0` | Proposed | Narrow `resolveFinalizeOutcome`'s abort-scope demotion so a post-close `workspace-setup` re-run failure does not invert a root whose target already recorded a `deliverable` producer disposition |
