...
| `coordinator-reason-evidence-shape` | Proposed | Structural (shape-only) evidence check on the `gc.coordinator_outcome.producer_disposition` reason field, warn-only by default behind `GC_COORDINATOR_REASON_ENFORCE`, following the ADR-0009 work-record gate precedent |
