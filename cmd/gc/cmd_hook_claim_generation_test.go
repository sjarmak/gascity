...
func TestDoHookClaimDoesNotAdvanceClaimGenerationOnAdoption(t *testing.T) {
	previousRunner := hookClaimCommandRunnerWithEnvContext
	unexpectedCommandRuns := 0
	hookClaimCommandRunnerWithEnvContext = func(context.Context, map[string]string) beads.CommandRunner {
		return func(string, string, ...string) ([]byte, error) {
			unexpectedCommandRuns++
			return nil, errors.New("test attempted to invoke a real command")
		}
	}
	t.Cleanup(func() { hookClaimCommandRunnerWithEnvContext = previousRunner })
		...
		ReadWorkMeta: func(_ context.Context, _ string, _ []string, beadID, assignee string) (beads.Bead, error) {
			...
			return beads.Bead{
				...
				Metadata: map[string]string{
					...
					beadmeta.ClaimGenerationMetadataKey: "3",
	...
	if readWorkMetaCalls != 2 {
		t.Fatalf("ReadWorkMeta calls = %d, want 2 (ownership certification and post-stamp readback)", readWorkMetaCalls)
	}
	if unexpectedCommandRuns != 0 {
		t.Fatalf("real command runner calls = %d, want 0; the adoption fixture must stay hermetic", unexpectedCommandRuns)
	}
}
