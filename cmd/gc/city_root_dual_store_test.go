...
func TestMixedCityRootControlReadySourcesSeeAlternateStore(t *testing.T) {
	testCityRootEnvs(t, func(t *testing.T, env cityRootTestEnv) {
		...
		sources, err := controlReadyCacheSources(cityPath, cityPath, cfg)
		if err != nil {
...
func TestMixedCityRootControlLedgerUsesExistingResolutionBeforeAlternatePrefix(t *testing.T) {
	testCityRootEnvs(t, func(t *testing.T, env cityRootTestEnv) {
		...
		if !sameExecutionStore(fileOwner, store) || gotFile.ID != fileBead.ID {
...
func TestCityRootAltStoreOpensOnceAcrossMixedRootReaders(t *testing.T) {
	testCityRootEnvs(t, func(t *testing.T, env cityRootTestEnv) {
		...
		for range 3 {
			if _, err := controlReadyCacheSources(cityPath, cityPath, cfg); err != nil {
...
func claimOneMixedCityBead(t *testing.T, cityPath, beadID string) {
	...
	code := claimHookWork(cityPath, query, cityPath, nil, []hookStore{{dir: cityPath, storeRef: "city"}}, hookClaimOptions{
...
func TestSingleStoreCityRootRetainsExistingResults(t *testing.T) {
	testCityRootEnvs(t, func(t *testing.T, env cityRootTestEnv) {
		...
		if err != nil || !sameExecutionStore(owner, store) || resolved.ID != work.ID {
		...
		mixedSources, err := controlReadyCacheSources(mixedPath, mixedPath, mixedCfg)
