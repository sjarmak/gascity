...
func (d *nudgeEventDispatcher) runPass(sessionFilter string, retriesLeft int) {
	...
	deliver := func(target nudgeTarget, obs worker.LiveObservation) (bool, error) {
		ok, err := tryDeliverQueuedNudgesByPoller(target, store.Store, store.Store, sp, d.quiescence, obs)
		if ok || err != nil {
			return ok, err
		}
		if retriesLeft <= 0 {
	...
	if _, err := deliverPendingQueuedNudges(d.cityPath, cfg, store.Store, store.Store, sp, sessionBeads, sessionFilter, d.stderr, deliver); err != nil {
