...
// TestBeadsMetadataGuardedClearGuardABARaceIsSkipped exercises an ABA race on
// the guard self-CAS: a wrapper store flips the guard away from
// guard-expected and back before the second write's self-CAS call returns, so
// the underlying CAS observes a live mismatch (swapped=false) but the
// ApplyMetadataCAS readback finds guard-expected again (already_next, not
// conflict). The sequence must still stop and report "skipped" with only the
// first key written, exactly like a persistent guard mismatch.
func TestBeadsMetadataGuardedClearGuardABARaceIsSkipped(t *testing.T) {
	clearGCEnv(t)
	configureIsolatedRuntimeEnv(t)
	...
	backing, id := newGuardedClearTestMemStore(t)
	store := &guardedClearRaceStore{MemStore: backing, id: id, guardKey: "gc.worktree_attempt_id"}
	store.abaGuardOnNthGuardCheck = 2
	store.abaGuardExpected = "att-1"
	installGuardedClearStoreSeams(t, func(_, _ string) (beads.Store, error) {
		return store, nil
	}, func(beads.Store) error { return nil })
	...
	if strings.Join(result.KeysWritten, ",") != "gc.work_dir" {
		t.Fatalf("keys_written=%v, want only the key written before the ABA race", result.KeysWritten)
	}

	got, err := backing.Get(id)
	if err != nil {
		t.Fatalf("Get after ABA skip: %v", err)
	}
	if got.Metadata["gc.work_branch"] != "/work/branch" {
		t.Fatalf("second key cleared despite ABA guard race: metadata=%v", got.Metadata)
	}
}

// TestBeadsMetadataGuardedClearRejectsGuardKeyAsWriteTarget exercises the
// validation guarantee that the guard key is never itself a write target:
// supplying it as --clear-key or --set-metadata must fail before any store
// access, not silently clear the fence it is meant to check.
func TestBeadsMetadataGuardedClearRejectsGuardKeyAsWriteTarget(t *testing.T) {
	clearGCEnv(t)
	configureIsolatedRuntimeEnv(t)

	cityPath := writeMetadataCASTestCity(t)
	_, id := newGuardedClearTestMemStore(t)

	_, stderr, code := runGuardedClearTestCommand(cityPath, id,
		"--guard-key=gc.worktree_attempt_id", "--guard-expected=att-1",
		"--clear-key=gc.worktree_attempt_id",
	)
	if code == 0 {
		t.Fatalf("code=0, want nonzero for --clear-key=guard-key; stderr=%q", stderr)
	}
	if !strings.Contains(stderr, "guard-key") {
		t.Fatalf("stderr=%q, want it to name the guard-key conflict", stderr)
	}

	_, stderr, code = runGuardedClearTestCommand(cityPath, id,
		"--guard-key=gc.worktree_attempt_id", "--guard-expected=att-1",
		"--set-metadata=gc.worktree_attempt_id=att-2",
	)
	if code == 0 {
		t.Fatalf("code=0, want nonzero for --set-metadata=guard-key=...; stderr=%q", stderr)
	}
	if !strings.Contains(stderr, "guard-key") {
		t.Fatalf("stderr=%q, want it to name the guard-key conflict", stderr)
	}
...
// its race modes is armed per test: reprovisionGuardOnNthGuardCheck rewrites
// the guard key before the Nth guard self-CAS; abaGuardOnNthGuardCheck
// rewrites the guard away from guard-expected right before the Nth guard
// self-CAS (so the underlying CAS observes a live mismatch and returns
// swapped=false) and then, on the very next Get (the readback
// ApplyMetadataCAS performs to classify that false result), restores the
// guard to guard-expected — reproducing an ABA race that must classify as
// already_next, not conflict; tamperKey/tamperValue rewrites a single named
// key before its own CAS call.
type guardedClearRaceStore struct {
	*beads.MemStore
	...
	abaGuardOnNthGuardCheck int
	abaGuardExpected        string
	abaGuardCheckCount      int
	abaPendingRestore       bool
...
func (s *guardedClearRaceStore) CompareAndSetMetadataKey(id, key, expected, next string) (bool, error) {
	...
	if s.abaGuardOnNthGuardCheck > 0 && key == s.guardKey && expected == next {
		s.abaGuardCheckCount++
		if s.abaGuardCheckCount == s.abaGuardOnNthGuardCheck {
			if err := s.SetMetadata(id, s.guardKey, "att-aba-intruder"); err != nil {
				return false, err
			}
			s.abaPendingRestore = true
		}
	}
...
// Get restores the guard to abaGuardExpected on the read immediately
// following an armed ABA-race CAS, modeling the intruder's own revert
// landing in the window between the failed self-CAS and ApplyMetadataCAS's
// readback.
func (s *guardedClearRaceStore) Get(id string) (beads.Bead, error) {
	if s.abaPendingRestore {
		s.abaPendingRestore = false
		if err := s.SetMetadata(id, s.guardKey, s.abaGuardExpected); err != nil {
