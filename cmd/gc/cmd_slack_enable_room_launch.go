...
// newSlackEnableRoomLaunchCmd returns `gc slack enable-room-launch` —
...
// `gc slack disable-room-launch` for removal.
func newSlackEnableRoomLaunchCmd(stdout io.Writer) *cobra.Command {
	var (
		workspaceID  string
		poolTemplate string
	)
...
// slackMapRigRestartReminder line is shared with the sibling map-channel
...
func runSlackEnableRoomLaunch(stdout io.Writer, channelID, workspaceID, poolTemplate string) error {
	...
	reg, err := newSlackRoomLaunchMappingRegistry(slackRoomLaunchMappingsPath(cityPath))
	if err != nil {
