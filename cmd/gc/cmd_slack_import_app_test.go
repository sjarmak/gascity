...
func TestSlackImportAppHappyPath(t *testing.T) {
	cityRoot := newTestCity(t)
	manifestPath := writeManifest(t, t.TempDir(), canonicalManifest())

	_, stderr, err := execSlackImportAppCmd(t, cityRoot,
		manifestPath,
		"--workspace-id", "T123",
		"--app-id", "A456",
	)
	...
	if !slackTestSliceHasAll(rec.Scopes, []string{"commands", "chat:write", "chat:write.customize"}) {
...
func TestSlackImportAppExtractsSlashCommands(t *testing.T) {
	cityRoot := newTestCity(t)
	body := []byte(`{
  ...
  "oauth_config": {
	...
	reg, _ := newSlackAppRegistry(slackAppsRegistryPath(cityRoot))
	rec, ok := reg.Get("T1", "A1")
	if !ok {
		t.Fatal("record missing")
	}
	if !slackTestSliceHasAll(rec.SlashCommands, []string{"/gc", "/gc-status"}) {
		t.Errorf("SlashCommands missing expected entries: %v", rec.SlashCommands)
	}
}

func TestSlackImportAppMalformedJSON(t *testing.T) {
	cityRoot := newTestCity(t)
...
func TestSlackImportAppMissingRequiredScope(t *testing.T) {
	cityRoot := newTestCity(t)
...
func TestSlackImportAppMissingRequiredFlag(t *testing.T) {
	cityRoot := newTestCity(t)
...
func TestSlackImportAppIdempotentReimport(t *testing.T) {
	cityRoot := newTestCity(t)
	p := writeManifest(t, t.TempDir(), canonicalManifest())
...
func TestSlackImportAppRegistryPathIsCityRootedFromNestedCwd(t *testing.T) {
	cityRoot := newTestCity(t)
	...
	_, stderr, err := execSlackImportAppCmd(t, nested,
		p, "--workspace-id", "T1", "--app-id", "A1",
	)
	if err != nil {
	...
	reg, err := newSlackAppRegistry(want)
	if err != nil {
		t.Fatal(err)
	}
...
func TestSlackImportAppRejectsOversizeManifest(t *testing.T) {
	cityRoot := newTestCity(t)
	...
	_, _, err := execSlackImportAppCmd(t, cityRoot,
		p, "--workspace-id", "T1", "--app-id", "A1",
	)
	if err == nil {
...
func TestSlackImportAppManifestNotFound(t *testing.T) {
	cityRoot := newTestCity(t)
...
func TestSlackImportAppForwardCompatUnknownFields(t *testing.T) {
	cityRoot := newTestCity(t)
	body := canonicalManifest()
...
func TestParseSlackManifestExtractsFields(t *testing.T) {
	...
	if !slackTestSliceHasAll(got.OAuthConfig.Scopes.Bot, []string{"commands", "chat:write"}) {
...
func slackTestSliceHasAll(haystack, needles []string) bool {
