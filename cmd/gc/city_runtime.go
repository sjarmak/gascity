...
// reloadOrderDispatchMuTimeout bounds how long config reload will wait to
// acquire orderDispatchMu before giving up on refreshing the order set this
// cycle. The order-dispatch lane can hold this mutex for as long as its
// slowest condition-trigger check_timeout permits (#2604/#3368) — reload
// must not block the tick goroutine behind that indefinitely. A timed-out
// acquire is not a failure: cr.cfg has already been updated by the time this
// runs, so the lane's own next scheduled pass re-scans it and converges
// independently, and shutdown's own order drain compensates for any
// dispatcher this left un-rebuilt.
const reloadOrderDispatchMuTimeout = 500 * time.Millisecond

// orderDispatchMuPollInterval is the retry cadence tryLockOrderDispatchMu
// uses while waiting on orderDispatchMu.
const orderDispatchMuPollInterval = 10 * time.Millisecond

// orderDispatchLaneJoinTimeout bounds how long run() waits for the
// order-dispatch lane goroutine to exit after ctx is canceled before
// proceeding to shutdown() regardless. This join is registered after `defer
// cr.shutdown()`, so by defer LIFO order it runs BEFORE shutdown() — an
// unbounded wait here would block session shutdown behind a wedged order
// exec (#2604/#3368). Sized above typical subprocess cancellation grace
// (shellExecSignalGrace + shellExecPostCancelWaitDelay) so the common case
// still joins cleanly; a lane that outlives this timeout leaks its goroutine
// for the remainder of process shutdown, and any tracking bead it left open
// is closed by sweepOrphanedOrderTrackingRetry on the next boot. A var (not
// const) so tests can shrink it instead of waiting out the real duration
// (#2604/#3368 review followup); see also orderRescanInterval below.
var orderDispatchLaneJoinTimeout = 10 * time.Second
...
type CityRuntime struct {
	...
	// orderDispatchConfigVersion increments every time reloadConfigTraced
	// installs a new cr.cfg (see the serviceStateMu-guarded write there).
	// Guarded by serviceStateMu, written in the SAME critical section as
	// cr.cfg, so a reader taking both under one RLock always gets a matched
	// (cfg, version) pair. A standalone atomic pending flag cannot make that
	// promise: reading it separately from cr.cfg means its true value is not
	// guaranteed to correspond to whichever cr.cfg snapshot sits beside it
	// (#2604/#3368 review followup).
	orderDispatchConfigVersion uint64

	// orderDispatchAppliedConfigVersion records the orderDispatchConfigVersion
	// baked into the currently active cr.od (config-only fields like
	// Orders.MaxDispatchesPerTick/MaxTimeout ride along in cfg but are not
	// part of orderSetSignature, so a config-only change needs its own
	// staleness signal). Guarded by orderDispatchMu, like orderSetAppliedSeq.
	// A rebuild is owed whenever a caller's (cfg, version) pair has version >
	// this field; applying one only ever raises this field to the version it
	// captured (never past it), so a fresher version left behind by a
	// concurrent reload while an older rebuild is still in flight remains
	// visibly owed to the next consumer instead of being silently erased —
	// the lost-notification failure mode a plain boolean pending flag cannot
	// avoid (#2604/#3368 review followup).
	orderDispatchAppliedConfigVersion uint64
	...
	// below (orderSweepWatchdogLast and friends): those are touched only
	// from within dispatchOrders, which only the lane goroutine ever calls,
	// so they need no lock. Reload's own acquires of this mutex are bounded
	// (reloadOrderDispatchMuTimeout, via tryLockOrderDispatchMu) rather than
	// blocking, since the lane can hold it for as long as a dispatch pass's
	// slowest condition-trigger check_timeout permits.
	...
	// against it.
	//
	// Guarded by its OWN dedicated mutex (orderDispatchLaneDoneMu), NOT
	// orderDispatchMu: dispatchOrders holds orderDispatchMu across the whole
	// od.dispatch() call, which can run for as long as a dispatch pass's
	// slowest condition-trigger check_timeout permits (see dispatchOrders).
	// If shutdown() read this field under orderDispatchMu, it would block
	// there for that same unbounded duration before ever reaching the
	// bounded select on the channel itself, defeating
	// orderDispatchLaneJoinTimeout entirely (#2604/#3368 review followup).
	orderDispatchLaneDoneMu sync.Mutex
...
func (cr *CityRuntime) run(ctx context.Context) {
	...
	defer func() {
		select {
		case <-orderDispatchDone:
		case <-time.After(orderDispatchLaneJoinTimeout):
			fmt.Fprintf(cr.stderr, "%s: order-dispatch lane did not exit within %s; proceeding with shutdown\n", //nolint:errcheck // best-effort stderr
...
func (cr *CityRuntime) tick(
	...
	orderAt, orderRan, orderStatusOK := cr.orderDispatchLaneStatus()
	addOrderDispatchAgeFields(orderFields, orderAt, orderRan, orderStatusOK)
...
//
// orderDispatchMu is held only around the dispatcher snapshot/dispatch call
// and the trailing status fields, not across the rescan-if-due scan or the
// tracking watchdogs: those touch fields the lane goroutine exclusively
// owns (rescanOrderDispatcherIfDue's own staleness check is separately
// guarded), so serializing them against reload buys nothing and only grows
// how long reload can be blocked waiting for this mutex. od.dispatch()
// itself must stay under the lock: memoryOrderDispatcher.carryLastRunCacheFrom
// (and its gate-backoff/open-work-suppression siblings) require the outgoing
// dispatcher's dispatch() call to have fully returned before a concurrent
// drain-and-replace reads its cache, and drain() alone does not wait for
// dispatch()'s synchronous phase — only for goroutines it has already
// spawned. Reload's own acquire of this mutex is bounded
// (reloadOrderDispatchMuTimeout) precisely because dispatch() can hold it
// for a while.
...
// tryLockOrderDispatchMu acquires orderDispatchMu, giving up after
// reloadOrderDispatchMuTimeout or ctx cancellation. sync.Mutex has no
// built-in timeout, so this polls TryLock (Go 1.18+); the poll interval is
// small relative to the timeout so the added latency over a plain Lock() is
// negligible, and the common uncontended case succeeds on the first
// attempt. Every caller is on the tick goroutine bounding its wait behind a
// possibly-slow lane dispatch() pass (#2604/#3368), so the timeout is fixed
// rather than a parameter.
func (cr *CityRuntime) tryLockOrderDispatchMu(ctx context.Context) bool {
...
// addOrderDispatchAgeFields). This runs on every tick, not just reload, so
// it must never block: a blocking Lock() here would let a slow lane
// dispatch() pass delay session-reconcile on every tick, reintroducing
// exactly the coupling #2604/#3368 removed. ok is false when the lane
// currently holds orderDispatchMu; callers should report "unknown" rather
// than block or fabricate stale data.
func (cr *CityRuntime) orderDispatchLaneStatus() (at time.Time, ran bool, ok bool) {
	if !cr.orderDispatchMu.TryLock() {
		return time.Time{}, false, false
	}
	defer cr.orderDispatchMu.Unlock()
	return cr.orderDispatchLastRunAt, cr.orderDispatchRan, true
}
...
// from a fresh run, mirroring addBackstopAgeFields. ok is false when
// orderDispatchLaneStatus could not acquire orderDispatchMu without
// blocking (the lane is mid-dispatch); that is reported as its own distinct
// status rather than fabricating a ran/age value from zero data.
func addOrderDispatchAgeFields(fields map[string]any, at time.Time, ran bool, ok bool) {
	if !ok {
		fields["order_dispatch_status"] = "unknown_lane_busy"
		return
	}
	if !ran {
...
func (cr *CityRuntime) rescanOrderDispatcherIfDue(ctx context.Context, cityRoot string, now time.Time) {
	...
	// orderRescanLast is also read and written by the reload paths under
	// orderDispatchMu (applyOrderSetSnapshotLocked); guard this staleness
	// check the same way now that it is no longer covered by dispatchOrders
	// holding the mutex across this whole call.
	cr.orderDispatchMu.Lock()
	due := cr.orderRescanLast.IsZero() || now.Sub(cr.orderRescanLast) >= orderRescanInterval
	cr.orderDispatchMu.Unlock()
	if !due {
		return
	}
	if _, err := cr.rescanOrderDispatcher(ctx, cityRoot, "gc patrol: order scan", now); err != nil {
		cr.orderDispatchMu.Lock()
...
// rescanOrderDispatcher scans the order set from disk and, if changed,
...
// unlike config reload (see rescanOrderDispatcherForReload). cfg and its
// version are captured together, under a single serviceStateMu.RLock(), so
// the version passed to applyOrderSetSnapshotLocked always identifies the
// exact cfg that was actually used to scan (#2604/#3368 review followup,
// sixth pass).
func (cr *CityRuntime) rescanOrderDispatcher(ctx context.Context, cityRoot string, cmdName string, now time.Time) (bool, error) {
	if ctx == nil {
	...
	cr.orderDispatchMu.Lock()
	defer cr.orderDispatchMu.Unlock()
	changed, _ := cr.applyOrderSetSnapshotLocked(ctx, cityRoot, snapshot, seq, cfgVersion, now)
	return changed, nil
}

// rescanOrderDispatcherForReload is rescanOrderDispatcher's config-reload
// counterpart (the unchanged-config-revision reload path). Reload runs on
// the tick goroutine, so it bounds its wait for orderDispatchMu instead of
...
// and converges independently. cfg and its version are captured together,
// under a single serviceStateMu.RLock(), from the canonical cr.cfg — never
// from a reload result's own parsed config, which can differ from cr.cfg in
// dispatcher-relevant fields despite a matching revision hash (#2604/#3368
// review followup, fifth pass) — so the version passed to
// applyOrderSetSnapshotLocked always identifies the exact cfg that was
// actually used to scan (sixth pass).
func (cr *CityRuntime) rescanOrderDispatcherForReload(ctx context.Context, cityRoot string, cmdName string, now time.Time) (bool, string, error) {
	cr.serviceStateMu.RLock()
	cfg := cr.cfg
	cfgVersion := cr.orderDispatchConfigVersion
	cr.serviceStateMu.RUnlock()
	...
	defer cr.orderDispatchMu.Unlock()
	changed, summary := cr.applyOrderSetSnapshotLocked(ctx, cityRoot, snapshot, seq, cfgVersion, now)
...
//
// scanCfgVersion is the orderDispatchConfigVersion the caller observed
// (under serviceStateMu) at the moment it captured the cfg used for THIS
// snapshot's disk scan (scanOrderSetSnapshotFS bakes cfg-dependent fields —
// formula layers, skips, timezone, per-order overrides — into snapshot.Orders
// at scan time). If a reload has installed a newer cr.cfg/version in the
// window between that scan and this call acquiring orderDispatchMu, building
// from the live cfg read below would combine old-generation orders with
...
func (cr *CityRuntime) reloadConfigTraced(
	...
	if cr.configRev != "" && result.Revision == cr.configRev {
		// result.Cfg is never used here — it is NOT assumed content-equivalent
		// to cr.cfg despite the matching Revision, since config loading applies
		// .gc/site.toml bindings outside what Revision hashes
		// (internal/config/compose.go), so the two can differ in
		// dispatcher-relevant fields even on a revision match.
		// rescanOrderDispatcherForReload sources the canonical cr.cfg (and its
		// paired orderDispatchConfigVersion) internally, so this scan and
		// whatever applyOrderSetSnapshotLocked builds are always consistent
		// with each other (#2604/#3368 review followup, fifth and sixth
		// passes).
		ordersChanged, orderSummary, orderErr := cr.rescanOrderDispatcherForReload(ctx, cityRoot, "gc reload: order scan", time.Now())
		if orderErr != nil {
	...
	// Install nextCfg and bump orderDispatchConfigVersion together, in the
	// SAME serviceStateMu critical section: a concurrent lane rescan
	// (rescanOrderDispatcher, called from rescanOrderDispatcherIfDue) takes
	// its own (cfg, version) pair under one RLock of this same mutex, so the
	// pair it reads is always consistent — it can never observe a version
	// number without the cr.cfg it actually belongs to, the failure mode a
	// standalone pending flag (read independently of cr.cfg) could not rule
	// out (#2604/#3368 review followup).
	...
	// the store they were scheduled against. The reload budget is capped at
	// reloadOrderDrainTimeout so a wedged exec order cannot stall the tick
	// loop; timed-out dispatchers are retained and drained again during
	// shutdown. Deriving from ctx (the tick ctx) lets a shutdown racing with
	// reload short-circuit the drain instead of waiting the full 1s.
	//
	...
	// drain reads cr.od too. The lane can hold this mutex for as long as its
	// slowest condition-trigger check_timeout permits, so reload bounds its
	// acquire (reloadOrderDispatchMuTimeout) instead of blocking the tick
	...
	if cr.tryLockOrderDispatchMu(ctx) {
		if seq <= cr.orderSetAppliedSeq {
			...
			// installing nextOD now would revert that newer state. Leave
			// orderDispatchAppliedConfigVersion untouched: it stays behind
			// cfgVersion, so the version comparison in
			// applyOrderSetSnapshotLocked (or this function's own next
			// reload pass) sees the rebuild as still owed and retries it
			// from a fresh cr.cfg snapshot — no separate pending bit to
			// remember to set (#2604/#3368 review followup).
			cr.orderDispatchMu.Unlock()
		} else {
			if cr.od != nil {
				drainCtx, drainCancel := context.WithTimeout(ctx, reloadOrderDrainTimeout)
			...
			// nextOD was built from nextCfg (this reload's config), so
			// cfgVersion is now baked into cr.od. Max-clamp rather than
			// assign: a concurrent winner using a higher cfgVersion could
			// already have advanced this field past ours while we were
			// still building nextOD.
			if cfgVersion > cr.orderDispatchAppliedConfigVersion {
				cr.orderDispatchAppliedConfigVersion = cfgVersion
			}
			cr.orderDispatchMu.Unlock()
		...
		// inline so the tick is not blocked. orderDispatchAppliedConfigVersion
		// is left behind cfgVersion (already installed in cr.cfg above), so
		// the next scan that acquires orderDispatchMu (lane or reload) sees
		// the gap and rebuilds from its own fresh cr.cfg regardless of
		// signature — the version comparison makes this automatic, unlike
		// the boolean pending flag it replaces (#2604/#3368 review followup).
...
// drainOrderDispatchers waits for in-flight dispatch goroutines on the
// current and retired dispatchers to finish, bounded by ctx. Called from
// shutdown() with the tick's own goroutine gone but the order-dispatch lane
// goroutine potentially still running past orderDispatchLaneJoinTimeout
// (#2604/#3368), so cr.od/cr.retiredOrderDispatchers are read and
// written under orderDispatchMu rather than assumed quiescent. The mutex
// acquisition itself is bounded (reloadOrderDispatchMuTimeout): a process
// exiting does not need a perfect drain, and any tracking bead a skipped
// drain leaves open is closed by sweepOrphanedOrderTrackingRetry on the
// next boot.
func (cr *CityRuntime) drainOrderDispatchers(ctx context.Context) {
	if !cr.tryLockOrderDispatchMu(ctx) {
		fmt.Fprintf(cr.stderr, "%s: order dispatch busy at shutdown; leaving drain to next boot's orphan sweep\n", cr.logPrefix) //nolint:errcheck // best-effort stderr
		return
	}
	od := cr.od
...
func (cr *CityRuntime) shutdown() {
	cr.shutdownOnce.Do(func() {
		...
		// followup). Read under orderDispatchLaneDoneMu, NOT orderDispatchMu:
		// the lane can hold orderDispatchMu for the whole od.dispatch() call,
		// and blocking here on that mutex would defeat the bounded select
		// below before it even starts (#2604/#3368 review followup).
		cr.orderDispatchLaneDoneMu.Lock()
		...
		defer func() {
			...
			if !laneExited {
				// The lane never confirmed exit within orderDispatchLaneJoinTimeout
				// and may still be mid-call against this binding (e.g. inside a
				// watchdog store call outside orderDispatchMu). Closing the engine
				// out from under an in-flight call risks a failed write, a race
				// inside the store's own Close, or a second unbounded wait there —
				// all worse than leaving the descriptor open for process exit to
				// reclaim (#2604/#3368 review followup). This does leak the binding
				// for the remainder of this process's life in the rare case the
				// lane never exits; that is the accepted cost of never closing a
				// store a caller might still be using.
				fmt.Fprintf(cr.stderr, "%s: order-dispatch lane still running past shutdown; leaving the storage binding open rather than closing it out from under it\n", cr.logPrefix) //nolint:errcheck // best-effort stderr
				return
			}
		...
		// Bounded like drainOrderDispatchers below: the order-dispatch lane
		// goroutine may still be running past orderDispatchLaneJoinTimeout in
		// the pathological case, and this check must not itself become the
		// thing that hangs shutdown.
		hasOrderDispatchers := true
		if cr.tryLockOrderDispatchMu(context.Background()) {
			hasOrderDispatchers = cr.od != nil || len(cr.retiredOrderDispatchers) > 0
			cr.orderDispatchMu.Unlock()
		}
		if hasOrderDispatchers {
