...
// nudgeHerdrLivePaneID resolves the pane id for an agent name via the herdr CLI's
// JSON envelope output. Returns "" when the agent is not (yet) listed.
func nudgeHerdrLivePaneID(t *testing.T, herdrSession, agentName string) string {
	t.Helper()
...
func herdrLiveReportAgent(t *testing.T, herdrSession, agentName, state string) {
	...
	var lastErr string
	for time.Now().Before(deadline) {
		paneID := nudgeHerdrLivePaneID(t, herdrSession, agentName)
		if paneID != "" {
