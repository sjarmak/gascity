...
func claimHookWorkWithRunner(workQuery, workDir string, queryEnv []string, stores []hookStore, claimOpts hookClaimOptions, ops hookClaimOps, run hookStoreRunner, emitFailure func(command string, err error), stdout, stderr io.Writer) int {
	...
	for len(remaining) > 0 {
		...
		activePriority = latchClaimBand(activePriority, selectedRank)
		...
		activePriority = latchClaimBand(activePriority, claimRank)
		storeOpts := claimOpts
...
func latchClaimBand(current *int, rank hookClaimCandidateRank) *int {
	if current != nil || rank.tier != 2 {
		return current
	}
