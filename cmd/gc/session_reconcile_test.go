...
// TestReconcile_InterruptedCloseStateReaped verifies that an open,
// pool-managed session bead stamped with a terminal ClosePatch stateCode the
// reconciler doesn't otherwise process (an interrupted close: state written,
// status flip lost) is closed rather than skipped forever. Before the fix
// these beads spun in the unknown-state skip on every tick, permanently
// holding their agent's pool slot, and gc restart could not recover because
// bd is the source of truth (#2085).
func TestReconcile_InterruptedCloseStateReaped(t *testing.T) {
	// Every stateCode in interruptedCloseSessionStates must reap: each can
	// only land on a pool-managed bead at its sole close write site, and all
	// share the identical guarded-close path. Iterate the production set
	// directly so a future addition is covered without a parallel list that
	// can silently drift.
...
// reap defers when the store view is degraded: a partial view is exactly
// when a false "no assigned work" read is most likely, so no closes may
// fire.
func TestReconcile_InterruptedCloseStateDeferredUnderPartialStore(t *testing.T) {
	...
	reconcileSessionBeads(
		...
		env.store, nil, nil, nil, env.dt, map[string]int{"worker": 1}, true, nil, "",
...
func TestReconcile_InterruptedCloseStateDeferredOnBoot(t *testing.T) {
	...
	cfgNames := configuredSessionNames(env.cfg, "", env.store)
	opts := append(append([]startExecutionOption{}, env.startOptions...), withDeferSessionClosesOnBoot())
	reconcileSessionBeads(
		context.Background(), []beads.Bead{session}, env.desiredState, cfgNames, env.cfg, env.sp,
...
// interrupted-close stateCode is NOT reaped — it falls through to the skip,
// so a named session never loses its identity to the reap path even if a
// future write stamped one of these reasons onto its state field. The reap
// is scoped to pool slots, whose loss is a cheap respawn. Runtime is not
// alive here, so the pool-managed guard is the sole reason the bead is kept
// open.
func TestReconcile_InterruptedCloseStateNamedSessionNotReaped(t *testing.T) {
	...
	// No pool_managed metadata -> isPoolManagedSessionInfo is false.
	session := env.createSessionBead("worker", "worker")
