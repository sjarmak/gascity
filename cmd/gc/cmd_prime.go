	...
	for _, a := range resolvedAgents {
		...
		if resolved, rErr := config.ResolveProvider(&a, &cfg.Workspace, cfg.Providers, exec.LookPath); rErr == nil && hookMode {
			...
			if hookSPErr != nil {
				...
				// the hook or drop the poller spawn. A nil provider preserves
				// the legacy sidecar spawn (providerRetiresNudgePollers treats
				// nil as not event-capable); only the event-capable suppression
				// is lost this pass.
