...
const (
	...
	drainAckCancelPendingLegacyKey  = "GC_DRAIN_ACK_CANCEL_TOKEN"
)

func drainAckCancelPendingKey(token string) string {
	return "GC_" + strings.ToUpper(sessions.DrainAckCancellationMetadataKey(token))
}
...
func retryRuntimeDrainAckCancellation(info sessions.Info, sessFront *sessions.Store, sp runtime.Provider, dt *drainTracker) (sessions.Info, bool) {
	...
	retryKey := drainAckCancelPendingKey(sessions.DrainAcknowledgementToken(info))
	token, err := sp.GetMeta(info.SessionNameMetadata, retryKey)
	legacyIndex := false
	if err == nil && strings.TrimSpace(token) == "" {
		token, err = sp.GetMeta(info.SessionNameMetadata, drainAckCancelPendingLegacyKey)
	...
	if err != nil {
		if errors.Is(err, sessions.ErrDrainAcknowledgementStopCommitted) {
			if dt != nil {
			...
			removeErr := sp.RemoveMeta(info.SessionNameMetadata, retryKey)
			if legacyIndex {
				removeErr = errors.Join(removeErr, sp.RemoveMeta(info.SessionNameMetadata, drainAckCancelPendingLegacyKey))
			}
			if removeErr != nil {
				return latest, false
			}
	...
	if sessions.DrainAcknowledgementToken(latest) != token && activeDurableAgentDrainAcknowledgement(latest) {
...
func clearDurableDrainAcknowledgement(info sessions.Info, sessFront *sessions.Store, dt *drainTracker, sp runtime.Provider) bool {
	...
	// Claim the shared token key before clearing it so a newer acknowledgement
	// either wins first or observes a resumable legacy-clear fence.
	if strings.TrimSpace(sessions.DrainAcknowledgementToken(info)) == "" {
		if strings.TrimSpace(info.DrainAckSource) == "" {
			return false
		}
		cleared, err := sessFront.ClearLegacyDrainAcknowledgement(info.ID)
		if err != nil {
	...
	if err != nil {
		...
		queueDurableDrainAcknowledgementClear(info, sp)
		return false
	}
	if !cleared {
...
func queueDurableDrainAcknowledgementClear(info sessions.Info, sp runtime.Provider) {
	token := sessions.DrainAcknowledgementToken(info)
	name := strings.TrimSpace(info.SessionNameMetadata)
	if sp == nil || name == "" || token == "" {
		return
	}
	if err := errors.Join(
		sp.SetMeta(name, drainAckCancelPendingKey(token), token),
		sp.SetMeta(name, drainAckCancelPendingLegacyKey, token),
	); err != nil {
		log.Printf("session wake: queueing provider-backed drain acknowledgement cancellation for %s: %v", info.ID, err)
	}
}
...
func advanceSessionDrainsWithSessionsTraced(
	...
	for id, ds := range dt.all() {
		...
		if !running {
			...
			if !completeStoppedDrain(info, sessFront, ds, completionScope, cfg, store,
				containsWakeReason(wakeEvals[id].Reasons, WakeWork), clk) {
				continue
			}
			dt.clearIdleProbe(id)
			dt.remove(id)
...
func completeDrain(info sessions.Info, sessFront *sessions.Store, ds *drainState, clk clock.Clock) {
	...
	completeDrainRefreshed(latest, sessFront, ds, false, clk)
}

// completeStoppedDrain refreshes durable acknowledgement provenance before
// deciding whether drain completion needs the expensive cross-store work scan.
func completeStoppedDrain(info sessions.Info, sessFront *sessions.Store, ds *drainState, scope drainCompletionWorkScope, cfg *config.City, store beads.Store, snapshotHasWork bool, clk clock.Clock) bool {
	if sessFront == nil {
		return true
	}
	latest, err := sessFront.RefreshDrainAcknowledgementInfo(info)
	...
	if activeDurableAgentDrainAcknowledgement(latest) {
		hasAssignedWork = drainCompletionHasAssignedWork(scope, cfg, store, latest, snapshotHasWork)
	}
	return completeDrainRefreshed(latest, sessFront, ds, hasAssignedWork, clk)
}

func completeDrainRefreshed(info sessions.Info, sessFront *sessions.Store, ds *drainState, hasAssignedWork bool, clk clock.Clock) bool {
	...
	_, err := sessFront.ApplyPatchClearingDrainAcknowledgementInfo(info, batch)
	return err == nil
}
