...
func cmdBeadsShowExecution(id, format string, stdout, stderr io.Writer) int {
	// The missing-id guard fires AFTER resolveReadTarget so a genuine resolve
	// error (e.g. a remote target without --city-name) still takes precedence,
	// matching the sibling cmdBeadsShow ordering (see newBeadsShowCmd).
	...
	// Warnings are emitted once by openAllConvoyStoresAt below; silence this
	// load's copy so the operator does not see every provenance warning twice.
	...
	sessions := newExecutionSessionLookup(cityPath, cfg, stderr)
...
// catalog cannot be constructed or listed, it prints the concrete reason to
// stderr and returns nil; execview.Build then omits the session section with a
// generic warning rather than failing the projection.
func newExecutionSessionLookup(cityPath string, cfg *config.City, stderr io.Writer) execview.SessionLookup {
	unavailable := func(reason string) execview.SessionLookup {
		fmt.Fprintf(stderr, "gc beads show --execution: live session state unavailable: %s\n", reason) //nolint:errcheck // best-effort stderr
		return nil
	}
...
func (gitWorktreeProbe) Probe(dir, baseRef string) execview.WorktreeView {
	...
	// Accumulate every probe error rather than silently leaving zero values,
	// which for ReachableFromMain would read identically to a confirmed
	// "not merged" answer. resolveWorktree surfaces v.Err as a warning.
	var errs []string
	if branch, err := g.CurrentBranch(); err == nil {
		v.Branch = branch
	} else {
		errs = append(errs, "current-branch: "+err.Error())
	}
	v.Dirty = g.HasUncommittedWork()
	head, err := g.Head()
	if err != nil {
		errs = append(errs, "head: "+err.Error())
...
func resolveIntegrationBase(g *git.Git, baseRef string) string {
	...
	// DefaultBranch probes origin/HEAD -> origin/main -> origin/master -> "main"
	// and never falls back to the checked-out branch (unlike ProbeDefaultBranch).
	// That matters: the target is a work worktree checked out on its feature
	// branch, so a current-branch fallback would make the branch its own base
	// and falsely report the work as merged (ahead=0, reachable_from_main=true).
...
func renderExecutionText(p execview.Projection, w io.Writer) {
	...
	if p.Worktree != nil {
		...
		default:
			...
			reachable := "unknown"
			if wt.ReachableFromMain != nil {
				reachable = fmt.Sprintf("%t", *wt.ReachableFromMain)
			}
			fp("    branch=%s  head=%s  dirty=%t  ahead=%s  reachable_from_main=%s\n",
				wt.Branch, shortSHA(wt.Head), wt.Dirty, ahead, reachable)
		}
	} else {
		fp("worktree: none\n")
	}
