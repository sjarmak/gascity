...
func TestSlackChannelMappingsPathIsCityRooted(t *testing.T) {
	cityRoot := newTestCity(t)
	got := slackChannelMappingsPath(cityRoot)
	want := filepath.Join(cityRoot, ".gc", "slack", "channel_mappings.json")
	if got != want {
		t.Errorf("slackChannelMappingsPath(%q) = %q, want %q", cityRoot, got, want)
	}
}

func TestSlackChannelMappingRegistryTolerantLoadOnMissingFile(t *testing.T) {
	cityRoot := newTestCity(t)
	reg, err := newSlackChannelMappingRegistry(slackChannelMappingsPath(cityRoot))
	if err != nil {
		t.Fatalf("newSlackChannelMappingRegistry on missing file: %v", err)
	}
...
func TestSlackChannelMappingRegistrySetAndGet(t *testing.T) {
	cityRoot := newTestCity(t)
	reg, err := newSlackChannelMappingRegistry(slackChannelMappingsPath(cityRoot))
	if err != nil {
		t.Fatal(err)
...
func TestSlackChannelMappingRegistryRequiredFields(t *testing.T) {
	cityRoot := newTestCity(t)
	reg, err := newSlackChannelMappingRegistry(slackChannelMappingsPath(cityRoot))
	if err != nil {
		t.Fatal(err)
	...
	now := time.Now().UTC()
	cases := []slackChannelMappingRecord{
...
func TestSlackChannelMappingRegistryRejectsUnknownTargetKind(t *testing.T) {
	cityRoot := newTestCity(t)
	reg, err := newSlackChannelMappingRegistry(slackChannelMappingsPath(cityRoot))
	if err != nil {
		t.Fatal(err)
	...
	now := time.Now().UTC()
	err = reg.Set(slackChannelMappingRecord{
...
func TestSlackChannelMappingRegistryRejectsCorruptFile(t *testing.T) {
	cityRoot := newTestCity(t)
	path := slackChannelMappingsPath(cityRoot)
	if err := os.MkdirAll(filepath.Dir(path), 0o700); err != nil {
		t.Fatal(err)
	}
	corrupt := map[string]slackChannelMappingRecord{
		"T1:C1": {
	...
	_, err := newSlackChannelMappingRegistry(path)
	if err == nil {
...
func TestSlackChannelMappingRegistryIdempotentOverwrite(t *testing.T) {
	...
	if err := reg.Set(slackChannelMappingRecord{
		WorkspaceID: "T1", ChannelID: "C1",
...
func TestSlackChannelMappingRegistryRemove(t *testing.T) {
	cityRoot := newTestCity(t)
	reg, err := newSlackChannelMappingRegistry(slackChannelMappingsPath(cityRoot))
	if err != nil {
		t.Fatal(err)
	...
	reg2, err := newSlackChannelMappingRegistry(slackChannelMappingsPath(cityRoot))
	if err != nil {
		t.Fatal(err)
	}
...
func TestSlackChannelMappingRegistryAllIsSortedByKey(t *testing.T) {
	cityRoot := newTestCity(t)
	reg, err := newSlackChannelMappingRegistry(slackChannelMappingsPath(cityRoot))
	if err != nil {
		t.Fatal(err)
...
func TestSlackChannelMappingRegistryFilePermissions(t *testing.T) {
	cityRoot := newTestCity(t)
	path := slackChannelMappingsPath(cityRoot)
	reg, err := newSlackChannelMappingRegistry(path)
	if err != nil {
		t.Fatal(err)
	}
	now := time.Now().UTC()
...
func TestSlackChannelMappingRegistryAtomicWriteCleansTmp(t *testing.T) {
	cityRoot := newTestCity(t)
	path := slackChannelMappingsPath(cityRoot)
	reg, err := newSlackChannelMappingRegistry(path)
	if err != nil {
...
func TestSlackChannelMappingRegistryConcurrentSets(t *testing.T) {
	cityRoot := newTestCity(t)
	path := slackChannelMappingsPath(cityRoot)
	reg, err := newSlackChannelMappingRegistry(path)
	if err != nil {
	...
	var roundtrip map[string]slackChannelMappingRecord
...
func TestSlackChannelMappingRegistryPersistsAndReloads(t *testing.T) {
	cityRoot := newTestCity(t)
	path := slackChannelMappingsPath(cityRoot)
	reg1, err := newSlackChannelMappingRegistry(path)
	if err != nil {
		t.Fatal(err)
	}
	now := time.Now().UTC()
	if err := reg1.Set(slackChannelMappingRecord{
		WorkspaceID: "T1", ChannelID: "C1",
	...
	reg2, err := newSlackChannelMappingRegistry(path)
	if err != nil {
