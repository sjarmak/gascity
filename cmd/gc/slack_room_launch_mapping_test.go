...
// TestSlackRoomLaunchMappingsPathIsCityRooted pins the on-disk path
// convention. The slack-pack adapter resolves the same path from
// GC_CITY_PATH at startup; both sides MUST agree.
func TestSlackRoomLaunchMappingsPathIsCityRooted(t *testing.T) {
	cityRoot := newTestCity(t)
	got := slackRoomLaunchMappingsPath(cityRoot)
	want := filepath.Join(cityRoot, ".gc", "slack", "room_launch_mappings.json")
	if got != want {
		t.Errorf("slackRoomLaunchMappingsPath(%q) = %q, want %q", cityRoot, got, want)
	}
}

// TestRoomLaunchRegistryEmptyOnMissing — tolerant load on a missing
// file yields an empty registry, matching every other slack-side
// registry (apps, channel, rig).
func TestRoomLaunchRegistryEmptyOnMissing(t *testing.T) {
	cityRoot := newTestCity(t)
	reg, err := newSlackRoomLaunchMappingRegistry(slackRoomLaunchMappingsPath(cityRoot))
	if err != nil {
		t.Fatalf("newSlackRoomLaunchMappingRegistry: %v", err)
	}
...
// TestRoomLaunchRegistrySetGetRoundtrip pins set/get via composite key.
func TestRoomLaunchRegistrySetGetRoundtrip(t *testing.T) {
	cityRoot := newTestCity(t)
	reg, err := newSlackRoomLaunchMappingRegistry(slackRoomLaunchMappingsPath(cityRoot))
	if err != nil {
		t.Fatalf("newSlackRoomLaunchMappingRegistry: %v", err)
	}
	rec := slackRoomLaunchMappingRecord{
		WorkspaceID:  "T1",
		ChannelID:    "C1",
...
// TestRoomLaunchRegistrySetPersistsCreatedAtAcrossReBind asserts the
// idempotent re-bind contract: re-binding the same channel preserves
// CreatedAt and advances UpdatedAt.
func TestRoomLaunchRegistrySetPersistsCreatedAtAcrossReBind(t *testing.T) {
	cityRoot := newTestCity(t)
	reg, err := newSlackRoomLaunchMappingRegistry(slackRoomLaunchMappingsPath(cityRoot))
	if err != nil {
		t.Fatal(err)
	}
	first := slackRoomLaunchMappingRecord{
		WorkspaceID:  "T1",
		ChannelID:    "C1",
	...
	// value matters.
	reg2, err := newSlackRoomLaunchMappingRegistry(slackRoomLaunchMappingsPath(cityRoot))
	if err != nil {
		t.Fatal(err)
	}
	second := slackRoomLaunchMappingRecord{
		WorkspaceID:  "T1",
		ChannelID:    "C1",
...
// TestRoomLaunchRegistrySetRejectsEmpty pins validation: every required
// field must be non-empty so the registry never holds a useless record.
func TestRoomLaunchRegistrySetRejectsEmpty(t *testing.T) {
	cityRoot := newTestCity(t)
	reg, err := newSlackRoomLaunchMappingRegistry(slackRoomLaunchMappingsPath(cityRoot))
	if err != nil {
		t.Fatal(err)
	}
	cases := []slackRoomLaunchMappingRecord{
...
// TestRoomLaunchRegistryRemove pins idempotent removal.
func TestRoomLaunchRegistryRemove(t *testing.T) {
	cityRoot := newTestCity(t)
	reg, _ := newSlackRoomLaunchMappingRegistry(slackRoomLaunchMappingsPath(cityRoot))
	_ = reg.Set(slackRoomLaunchMappingRecord{WorkspaceID: "T1", ChannelID: "C1", PoolTemplate: "p/q"})
...
// TestRoomLaunchRegistryTolerantLoadMissing — opening at a path that
// does not exist yields an empty registry, not an error. Matches every
// other slack registry's tolerant-load contract.
func TestRoomLaunchRegistryTolerantLoadMissing(t *testing.T) {
	tmp := t.TempDir()
	path := filepath.Join(tmp, "nope", "missing.json")
	reg, err := newSlackRoomLaunchMappingRegistry(path)
	if err != nil {
...
// TestRoomLaunchRegistryRejectsCorruptStore — a malformed JSON file is
...
func TestRoomLaunchRegistryRejectsCorruptStore(t *testing.T) {
	...
	_, err := newSlackRoomLaunchMappingRegistry(path)
	if err == nil {
