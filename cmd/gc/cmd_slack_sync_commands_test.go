...
func readRegistryRecord(t *testing.T, cityRoot, workspaceID, appID string) slackAppRecord {
	t.Helper()
...
// execSlackSyncCommandsCmd executes the verb in-process against a temp
...
func execSlackSyncCommandsCmd(t *testing.T, cityRoot, baseURL, token string, args ...string) (string, string, error) {
	t.Helper()
...
func TestSyncCommandsHappyPath_PushesUpdateAndVerifies(t *testing.T) {
	...
	stdout, stderr, err := execSlackSyncCommandsCmd(t, cityRoot, api.server.URL, api.expectedToken,
...
func TestSyncCommandsRegistryMiss_FailsBeforeAnyHTTPCall(t *testing.T) {
	...
	stdout, stderr, err := execSlackSyncCommandsCmd(t, cityRoot, api.server.URL, "xoxe.xoxp-token",
		"--workspace-id", "T1", "--app-id", "A1")
	if err == nil {
...
func TestSyncCommandsMissingToken_FailsBeforeAnyHTTPCall(t *testing.T) {
	...
	stdout, stderr, err := execSlackSyncCommandsCmd(t, cityRoot, api.server.URL, "",
		"--workspace-id", "T1", "--app-id", "A1")
	if err == nil {
...
func TestSyncCommandsDiffClassification_AddedRemovedChanged(t *testing.T) {
	...
	// Co-occurrence: each command name must appear under (or on the
	// same line as) the correct classification label, not just somewhere
	// in stdout. assertCommandClassification walks paragraph blocks
	// keyed by labels added/removed/changed.
...
//
// This is the single source of truth for the dry-run text format the
// tests pin. If the impl changes the format, update this parser
// alongside.
func parseDryRunSections(s string) map[string][]string {
	...
	for _, line := range strings.Split(s, "\n") {
		if m := labelRE.FindStringSubmatch(line); m != nil {
			...
			// Some formats put the command on the same line as the
			// label (e.g. "added: /foo"). Capture any inline cmd too.
...
func TestSyncCommandsVerifyFailure_AfterUpdate(t *testing.T) {
	...
	combined := err.Error() + stderr
	// Canonical phrase: "diverged" — pinned so callers can match on a
	// single string in error-handling logic.
...
func TestSyncCommandsTimeoutBudget_AppliesToHTTPCalls(t *testing.T) {
	cityRoot := newTestCity(t)
	api := &fakeSlackAPI{}
	// release lets the test goroutine unblock the handler on cleanup
	// instead of waiting on a wall-clock timer (CI-friendly: no
	// fixed-duration sleep). t.Cleanup is LIFO — register Server.Close
	// FIRST so close(release) fires BEFORE Close() tries to drain
	// active connections.
...
func TestSyncCommandsRefusesNonSlackBaseURL(t *testing.T) {
	...
	// execSlackSyncCommandsCmd's helper relax.
	t.Chdir(cityRoot)
	// Use https so we exercise the host-allowlist branch specifically;
	// http would short-circuit on the scheme check (which is also
	// covered by the security model — see isSlackAPIBaseURL).
...
func TestSyncCommandsRejectsUnknownOutputFormat(t *testing.T) {
	...
	stdout, _, err := execSlackSyncCommandsCmd(t, cityRoot, api.server.URL, "xoxe.xoxp-token",
