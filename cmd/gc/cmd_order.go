...
func doOrderRunWithJSON(aa []orders.Order, name, rig, cityPath string, store beads.OrdersStore, ep events.Provider, jsonOutput bool, vars map[string]string, stdout, stderr io.Writer) int {
	...
	recipe, wispVars, err := prepareOrderWispRecipe(context.Background(), genericStore, a, searchPaths, vars)
	if err != nil {
		fmt.Fprintf(stderr, "gc order run: %v\n", err) //nolint:errcheck // best-effort stderr
		return 1
	}
	...
	cookResult, err := molecule.Instantiate(context.Background(), genericStore, recipe, molecule.Options{Vars: wispVars})
	if err != nil {
		fmt.Fprintf(stderr, "gc order run: %v\n", err) //nolint:errcheck // best-effort stderr
		return 1
	}
