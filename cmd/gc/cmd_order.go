...
func newOrderSweepNudgeMailCmd(stdout, stderr io.Writer) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Short: "Close stale delivered nudge beads, read mail beads, and unread mail beads",
		Long: `Close stale delivered nudge beads, read mail beads, and unread mail beads.

Nudge beads that are past --nudge-ttl and not in the live nudge queue are
closed. Read mail beads past --mail-ttl are closed. Unread mail beads past
--unread-mail-ttl are also closed, on their own longer TTL and with a close
reason distinct from read mail's, so a swept-unread bead is never mistaken for
one a seat actually read. A budget cap of ` + fmt.Sprintf("%d", nudgeMailSweepCloseBudget) + ` closes
...
func cmdOrderSweepNudgeMailDryRun(nudges beads.NudgesStore, mail beads.MailStore, nudgeState *nudgequeue.State, now time.Time, nudgeTTL, mailTTL, unreadMailTTL time.Duration, quiet bool, stdout, stderr io.Writer) int {
	...
	if counts.NudgeClosed == 0 && counts.MailClosed == 0 && counts.UnreadMailClosed == 0 {
		fmt.Fprintln(stdout, "nudge-mail-sweep: nothing to close (0 stale nudge beads, 0 stale mail beads, 0 unread mail beads)") //nolint:errcheck // best-effort stdout
		return 0
	}
	fmt.Fprintf(stdout, "[DRY RUN] nudge-mail-sweep: would close %d nudge bead(s), %d mail bead(s), %d unread mail bead(s)  (no changes made)\n", //nolint:errcheck
		counts.NudgeClosed, counts.MailClosed, counts.UnreadMailClosed)
	return 0
}
...
func cmdOrderSweepNudgeMailRun(nudges beads.NudgesStore, mail beads.MailStore, nudgeState *nudgequeue.State, now time.Time, nudgeTTL, mailTTL, unreadMailTTL time.Duration, quiet bool, stdout, stderr io.Writer) int {
	...
	fmt.Fprintf(stdout, "nudge-mail-sweep: closed %d nudge bead(s), %d mail bead(s), %d unread mail bead(s)  %s\n", //nolint:errcheck // best-effort stdout
		result.NudgeClosed, result.MailClosed, result.UnreadMailClosed, budgetLine)
	return 0
}
