...
// replying to mail. When non-nil, it is called with the recipient name and the
// id of the mail message that triggered the notification. messageID becomes
// the caller-supplied durable-record identity for any queued nudge this
// produces (EFFECT-003), so a retried notification for the same message dedups
// instead of minting an unidentifiable duplicate. Errors are non-fatal.
