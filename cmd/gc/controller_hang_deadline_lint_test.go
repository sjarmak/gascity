...
var controllerTestExcludedHangDeadlineLines = map[int]string{
	455:  "input the test feeds a fake server to define the scenario, not a hang detector",
	911:  "negative-assertion window (asserts no watcher poke arrives)",
	961:  "negative-assertion window (asserts no watcher poke arrives, loop body)",
	1490: "bounded best-effort probe with no assertion on either branch",
}
