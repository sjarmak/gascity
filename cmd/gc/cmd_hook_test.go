...
func TestHookNoWork(t *testing.T) {
	...
	var stdout, stderr bytes.Buffer
	code := doHook("bd ready", "", false, nil, runner, &stdout, &stderr)
	if code != 1 {
...
func TestHookInjectNoWork(t *testing.T) {
	runner := func(string, string) (string, error) { return "", nil }
	var stdout, stderr bytes.Buffer
	code := doHook("bd ready", "", true, nil, runner, &stdout, &stderr)
	if code != 0 {
...
func TestHookPassesWorkQuery(t *testing.T) {
	...
	var stdout, stderr bytes.Buffer
	doHook("bd ready --assignee=mayor", "/tmp/work", false, nil, runner, &stdout, &stderr)
