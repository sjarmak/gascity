...
func TestWorkflowServeControlReadyQueryUsesControlTiers(t *testing.T) {
	...
	for _, want := range []string{
		`bd --readonly --sandbox ready --assignee="$cand" --exclude-type=epic`,
		`bd --readonly --sandbox ready --metadata-field "gc.routed_to=$GC_CONTROL_TARGET" --unassigned --exclude-type=epic`,
		`bd --readonly --sandbox ready --metadata-field "gc.routed_to=$GC_CONTROL_LEGACY_TARGET" --unassigned --exclude-type=epic`,
	} {
...
case "$*" in
  ...
  "--readonly --sandbox ready --assignee=gascity--control-dispatcher --exclude-type=epic --json --limit=20")
    printf '[{"id":"ga-ready"}]'
    ;;
  "--readonly --sandbox ready --metadata-field gc.routed_to=gascity/control-dispatcher --unassigned --exclude-type=epic --json --limit=20")
    printf '[{"id":"ga-routed"}]'
    ;;
  *)
    printf '[]'
...
`)
	...
	wantArgs := []string{"--readonly", "--sandbox", "ready", "--metadata-field", "gc.routed_to=my rig/control-dispatcher", "--unassigned", "--exclude-type=epic", "--json", "--limit=20"}
	if !slices.Equal(gotArgs, wantArgs) {
...
set -eu
case "$*" in
  "--readonly --sandbox ready --metadata-field gc.routed_to=gascity/workflow-control --unassigned --exclude-type=epic --json --limit=20")
