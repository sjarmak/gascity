...
// key, and stamping one would suppress legitimate repeat runs of a scheduled or
// event order. No-op for non-graph recipes or when no vars are supplied.
...
func (m *memoryOrderDispatcher) dispatchWisp(ctx context.Context, store beads.Store, target execStoreTarget, a orders.Order, cityPath, trackingID string, vars map[string]string) {
	...
	cookResult, err := molecule.Instantiate(ctx, store, recipe, molecule.Options{Vars: effectiveVars})
	if err != nil {
		m.rec.Record(events.Event{
			Type:    events.OrderFailed,
			Actor:   "controller",
