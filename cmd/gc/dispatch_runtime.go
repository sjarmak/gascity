...
func workflowServeControlReadyQuery(agentCfg config.Agent, controlSessionNames ...string) string {
	...
	query := queryPrefix + ` sh -c '` +
		...
		`[ -z "$cand" ] && continue; ` +
		`emit_ready bd --readonly --sandbox ready --assignee="$cand" --exclude-type=epic --json --limit=` + limit + `; ` +
		`done; ` +
		`done; ` +
		`emit_ready bd --readonly --sandbox ready --metadata-field "gc.routed_to=$GC_CONTROL_TARGET" --unassigned --exclude-type=epic --json --limit=` + limit + `; `
	if legacy := workflowServeLegacyControlRoute(target); legacy != "" {
		query += `emit_ready bd --readonly --sandbox ready --metadata-field "gc.routed_to=$GC_CONTROL_LEGACY_TARGET" --unassigned --exclude-type=epic --json --limit=` + limit + `; `
	} else {
		query += `:; `
	}
