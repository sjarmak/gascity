...
func TestServiceClientsPreferSupervisorForManagedCity(t *testing.T) {
	supervisorClient := api.NewCityScopedClient("http://supervisor.invalid", "bright-lights")
	oldAlive, oldSupervisor := serviceControllerAliveHook, serviceSupervisorClientHook
	serviceControllerAliveHook = func(string) int { return 1234 }
	serviceSupervisorClientHook = func(string) *api.Client { return supervisorClient }
	t.Cleanup(func() {
		serviceControllerAliveHook = oldAlive
		serviceSupervisorClientHook = oldSupervisor
	})

	cfg := &config.City{
		Workspace: config.Workspace{Name: "bright-lights"},
		API:       config.APIConfig{Bind: "127.0.0.1", Port: 1},
