...
| Medium owner | `cmd/gc` package `main` | TestMailDeliveryExactBinaryRecoversCommittedEffectAcrossCWD: subprocess | gc-snrfp.3 | the exact built-binary mail-delivery crash-recovery proof is a checked Medium subprocess owner; the built gc subprocesses are confined to TestMailDeliveryExactBinaryRecoversCommittedEffectAcrossCWD, which proves the production CLI recovers one committed destination effect across process and working-directory boundaries | gc-snrfp.3 | 2026-10-01 |
| Medium owner | `cmd/gc` package `main` | TestMailDeliveryInvokeAPIPreservesResultAndNeverFallsBackOnResponses: http_test_server | gc-dyfe8.1 | the CLI API mutation routing proof is a checked Medium loopback-server owner; the loopback server is confined to one result-bearing, business-error, and malformed-response no-fallback matrix and is closed by test cleanup | gc-dyfe8.1 | 2026-10-01 |
...
| Medium owner | `internal/api` package `api` | TestMailDeliveryControlPlaneRoundTrip: http_test_server | gc-dyfe8.1 | the running-supervisor mail-delivery generated-client roundtrip is a checked Medium loopback-server owner; the loopback server is confined to the generated client and typed result-bearing supervisor route proof and is closed by test cleanup | gc-dyfe8.1 | 2026-10-01 |
| Medium owner | `internal/api` package `api` | TestMailDeliveryMutationClientMalformedAndBusinessResponsesDoNotFallback: http_test_server | gc-dyfe8.1 | the mail-delivery generated-client malformed/business response matrix is a checked Medium loopback-server owner; the loopback server is confined to one object, string, number, null, 4xx, and 5xx non-fallback matrix and is closed by test cleanup | gc-dyfe8.1 | 2026-10-01 |
