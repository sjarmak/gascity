...
### A publication hold overrides the push step above (gc-dqv7wi)

The Session Completion workflow above is generated by `bd` and is written for
a rig that publishes its own work. This rig does not always publish its own
work, and where the two disagree, this section wins.

**If your work runs under a formula that declares a publication hold** -- the
rig sets `publication_hold`, read by `mol-scoped-work`, `mol-focus-review` and
`mol-adopt-pr` -- **or the bead, its comments, or your dispatch note say
branch-ready / no push / no PR, then step 4 does not apply to you.** Commit on
the work branch and stop there. Do not push, do not open a PR, and do not
treat "work is not complete until `git push` succeeds" as reaching you: on a
hold rig the push is a separate, gated step owned by someone else, not the
last step of yours. Leaving a reviewed branch unpushed is the correct terminal
state, not stranded work.

Ordering is why this has to be written here rather than left to the formula.
The formula enforces the hold at its **finalize** step, but you reach the end
of your implementation work before that step runs. If you consult the
generated workflow at that moment, it tells you to push, and the formula never
gets the chance to refuse. That is not hypothetical: on 2026-09-16 a worker
finished a dispatch, announced that it would now push per the mandatory
session workflow, and stopped only because it chose to ask first. The branch
it was about to push carried an unreviewed fix to a fleet-wide build break.

Asking is good; asking is not the safeguard. The safeguard is knowing that a
hold means you are done when the commit lands.
