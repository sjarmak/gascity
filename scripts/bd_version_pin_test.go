...
func TestBDVersionPins(t *testing.T) {
	...
	if goModMatch == nil {
		t.Fatal("go.mod missing an exact version pin for github.com/steveyegge/beads")
	}
	if got, want := goModMatch[1], bdCurrent; got != want {
		t.Fatalf("go.mod beads version = %q, want BD_CURRENT_VERSION %q", got, want)
	}
