...
3. **Dispatch:** `gc-sling <agent> <bead-id>` to route work. Use the
   wrapper (not raw `gc sling`) so beads matching rules in
   `.gc/sling-intercept.yaml` auto-get the right `--on <formula>`.
   Pass-through for anything that doesn't match a rule.
