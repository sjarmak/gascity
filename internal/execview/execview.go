...
type WorktreeView struct {
	...
	// the bead's recorded base ref, else the repo's default branch. Nil when it
	// could not be determined (no base resolved or a probe error), which is
	// deliberately distinct from a confirmed false.
...
func Build(stores []beads.Store, workBead beads.Bead, sessions SessionLookup, worktrees WorktreeProbe) Projection {
	...
	for _, r := range roots {
		...
		cur, extra := currentStep(steps, root.ID, ready)
		wf.CurrentStep = cur
		if extra > 0 {
			p.warn(fmt.Sprintf("workflow %s has %d concurrent steps at the current stage; showing lowest id", root.ID, extra+1))
		}
...
// many additional steps at the same stage were suppressed. The root bead and
// closed steps are excluded. Selection tiers, highest first: an in-progress
// (actively worked) step; a claimed-but-not-yet-running assigned step; a
// runnable (ready, unclaimed) step. Ties break on lowest ID. The extra count
// is reported symmetrically for whichever tier is chosen. Returns (nil, 0)
...
func currentStep(steps []beads.Bead, rootID string, ready map[string]bool) (*StepView, int) {
	...
	for _, tier := range []struct {
		...
		if len(tier.steps) > 0 {
			sortByID(tier.steps)
			sv := stepView(tier.steps[0], tier.runnable)
			return &sv, len(tier.steps) - 1
		}
	}
	return nil, 0
}
