...
func TestCurrentStep_MultipleAssignedReportsExtraSymmetrically(t *testing.T) {
	// No in-progress step, two claimed (assigned, open) steps: the assigned
	// tier must report the suppressed count too, not just the in-progress tier.
	steps := []beads.Bead{
		step("gc-b", "gc-root", "b", "open", "s2"),
		step("gc-a", "gc-root", "a", "open", "s1"),
	}
	cur, extra := currentStep(steps, "gc-root", nil)
	if cur == nil || cur.ID != "gc-a" {
		t.Fatalf("currentStep = %+v, want gc-a", cur)
	}
	if extra != 1 {
		t.Errorf("extra = %d, want 1 (assigned tier reports extra symmetrically)", extra)
	}
}
