...
// to an unbounded ReadFiltered, which walks archives FirstSeq-ascending, so on a city
...
// against ReadFiltered, whose Limit takes whichever match its FirstSeq-ascending
// walk reaches first -- the OLDEST for the disjoint ascending windows used here. A caller asking for
...
//
// What actually rejects the duplicate here is the SNAPSHOT GUARD, not the Seq
// dedupe in matchSet: the active tail supplies seq 5, so the ceiling is 5 and
// the archived seq 5 fails admits' strict 5 < 5. Mutating snapshotGuard.admits
// to accept everything turns this test red. The cross-archive Seq dedupe is
// guarded separately, by the overlapping-window test below.
//
// The duplicate and its predecessor share one archive, which is the shape the
// rotation window actually produces.
...
// TestReadFilteredNewestFirstSurvivesOverlappingArchiveWindows pins the Seq
// dedupe. Archive seq windows are supposed to be disjoint and descending, but
// nothing enforces that across archives: parseArchiveBasename validates only
// that each window is internally ordered. Two windows overlapping at seq 7 must
// not yield it twice, and the duplicate must not consume a result slot that the
// genuinely older match needs.
...
// two archives share LastSeq 100, so the basenames alone cannot say which holds
// the newer match. Both get opened unless the first supplies a match equal to
// that shared LastSeq, which proves the other cannot improve on it -- which is
// exactly the case below, and exactly why this fixture is the inert one here.
//
// What the pair guards is the LastSeq comparison in the continuation rule, NOT
// the walk order. Only ONE of the two orientations is live at a time, and WHICH
// one depends on the order tied archives happen to be visited in -- which the
// comparator deliberately does not pin, since it sorts on LastSeq alone. So the
// pair is kept together on purpose: whichever archive the walk reaches first,
// the other fixture is the one that goes red if the LastSeq comparison is
// dropped from the guard.
//
// Do not "clean this up" by deleting the orientation that currently passes
// under mutation. That was tried on this branch: at the time, the tie-break
// still ordered ties by FirstSeq descending, this fixture was the live one and
// the mirror looked inert, so the mirror was deleted. Deleting the tie-break in
// the same change inverted the visit order and with it which fixture was live,
// leaving the suite guarding nothing here. Re-measure both after any change to
// archive ordering, and keep both regardless.
//
// Order among LastSeq-TIED archives is what is unobservable from results here.
// The primary descending-LastSeq order is a different matter and is
// correctness-critical. That primary order -- and only that one -- is guarded
// at the open level by TestReadFilteredNewestFirstOpensTheHighestWindowFirst,
// whose fixture pairs LastSeq 100 against 60 rather than a tie. The tied case
// is guarded by this test and its mirror, and by nothing else.
...
//
// Be exact about which ordering is load-bearing, because the two are easy to
// conflate. The DESCENDING LastSeq order is correctness-critical: the break
// tests one archive's LastSeq against the lowest retained match, so under an
// arbitrary order a low window can trigger it before a higher one is reached
// ([1,50] matching at 50, then [1,40], breaks and never opens [1,100] matching
// at 100). Only among archives TIED on LastSeq is the order free, which is why
// the comparator carries no tie-break.
//
// Within that, what this test guards is which archives get OPENED, and no
// assertion on the returned events can see it: the retained matches are the
// same either way. An unreadable archive turns "was it opened" into a visible
// pass or fail, which is the only channel left.
//
// The nested [50,60] archive is unreadable. Ordering by LastSeq opens [1,100]
// first, retains seq 70, then stops because 60 <= 70, never touching it.
// Ordering by FirstSeq DESCENDING opens it first and the read fails.
...
// TestReadFilteredNewestFirstBreaksLastSeqTieMirrored is the other orientation
// of the tie above, with the newer match in the archive the walk reaches
// SECOND. See that test's comment for why both orientations are kept: exactly
// one of them is live against the continuation-rule mutation at any given
// archive ordering, and the comparator does not pin which.
func TestReadFilteredNewestFirstBreaksLastSeqTieMirrored(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "events.jsonl")
	...
	writeGzArchiveNamed(t, dir, "events.jsonl.archive-20260507T100000Z-seq-1-100.gz",
		controllerStartLine(90)+beadLine(100))
	writeGzArchiveNamed(t, dir, "events.jsonl.archive-20260507T110000Z-seq-50-100.gz",
		beadLine(50)+controllerStartLine(100))
	writeTypedJSONL(t, path, BeadCreated, 101)
	...
	if seqs := seqsOf(got); !reflect.DeepEqual(seqs, []uint64{100}) {
		t.Fatalf("seqs = %v, want [100]; the walk stopped after one tied archive instead of opening both", seqs)
	}
}
