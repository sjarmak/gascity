...
func TestBootstrapPolicyOwnsListenerHelperDebt(t *testing.T) {
	...
	if audit.BaselineCalls != 61 || audit.BaselineFiles != 23 || audit.ReportedCalls != 58 || audit.ReportedFiles != 23 {
		t.Fatalf("all-source listener-helper baseline/reported = %d/%d, %d/%d; want 61/23, 58/23", audit.BaselineCalls, audit.BaselineFiles, audit.ReportedCalls, audit.ReportedFiles)
	}
	...
	for _, rows := range [][]Baseline{bootstrapPolicy.Debt, bootstrapPolicy.SmallDebt} {
		...
		if row.BaselineCalls != 41 || row.BaselineFiles != 13 || row.ReportedCalls != 38 || row.ReportedFiles != 13 {
			t.Fatalf("listener-helper baseline/reported = %d/%d, %d/%d; want 41/13, 38/13", row.BaselineCalls, row.BaselineFiles, row.ReportedCalls, row.ReportedFiles)
		}
