...
var bootstrapPolicy = Ledger{
	...
	Medium: []MediumOwner{
		...
		},
		{
			PackageDir:      "cmd/gc",
			PackageName:     "main",
			Owner:           "TestMailDeliveryInvokeAPIPreservesResultAndNeverFallsBackOnResponses",
			Resources:       []Resource{ResourceHTTPTestServer},
			OwnerBead:       "gc-dyfe8.1",
			Invariant:       "the CLI API mutation routing proof is a checked Medium loopback-server owner",
			ResourceOwner:   "the loopback server is confined to one result-bearing, business-error, and malformed-response no-fallback matrix and is closed by test cleanup",
			MigrationTarget: "gc-dyfe8.1",
			Expires:         "2026-10-01",
		},
		{
			PackageDir:      "internal/api",
			PackageName:     "api",
			Owner:           "TestMailDeliveryMutationClientMalformedAndBusinessResponsesDoNotFallback",
			Resources:       []Resource{ResourceHTTPTestServer},
			OwnerBead:       "gc-dyfe8.1",
			Invariant:       "the mail-delivery generated-client malformed/business response matrix is a checked Medium loopback-server owner",
			ResourceOwner:   "the loopback server is confined to one object, string, number, null, 4xx, and 5xx non-fallback matrix and is closed by test cleanup",
			MigrationTarget: "gc-dyfe8.1",
			Expires:         "2026-10-01",
		},
		{
			PackageDir:      "internal/api",
			PackageName:     "api",
			Owner:           "TestMailDeliveryControlPlaneRoundTrip",
			Resources:       []Resource{ResourceHTTPTestServer},
			OwnerBead:       "gc-dyfe8.1",
			Invariant:       "the running-supervisor mail-delivery generated-client roundtrip is a checked Medium loopback-server owner",
			ResourceOwner:   "the loopback server is confined to the generated client and typed result-bearing supervisor route proof and is closed by test cleanup",
			MigrationTarget: "gc-dyfe8.1",
			Expires:         "2026-10-01",
		},
		{
			PackageDir:      "cmd/gc",
			PackageName:     "main",
			Owner:           "TestMailDeliveryExactBinaryRecoversCommittedEffectAcrossCWD",
			Resources:       []Resource{ResourceSubprocess},
			OwnerBead:       "gc-snrfp.3",
			Invariant:       "the exact built-binary mail-delivery crash-recovery proof is a checked Medium subprocess owner",
			ResourceOwner:   "the built gc subprocesses are confined to TestMailDeliveryExactBinaryRecoversCommittedEffectAcrossCWD, which proves the production CLI recovers one committed destination effect across process and working-directory boundaries",
			MigrationTarget: "gc-snrfp.3",
			Expires:         "2026-10-01",
		},
		{
			PackageDir:      "cmd/gc",
