...
func ReportAgent(t *testing.T, herdrSession, agentName, state string, pane func() string) {
	...
	Poll(t, fmt.Sprintf("forcing agent %q to state %q", agentName, state), 10*time.Second, func() (bool, string) {
		...
		out, err := reportAgentCmd(herdrSession, paneID, agentName, state).CombinedOutput()
		if err == nil {
			return true, ""
		}
...
func ReportAgentBestEffort(herdrSession, agentName, state string, pane func() string) {
	...
	_ = reportAgentCmd(herdrSession, paneID, agentName, state).Run()
}

func reportAgentCmd(herdrSession, paneID, agentName, state string) *exec.Cmd {
	return exec.Command("herdr", "--session", herdrSession, "pane", "report-agent", paneID,
