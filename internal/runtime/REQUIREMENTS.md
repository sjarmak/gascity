...
| RUNTIME-SEL-014 | Runtime selection transport identity | The runtime selection has one shared bundled-transport mapping while runtime and transport remain coupled: `acp` maps to `acp`; `t3bridge` and the legacy `exec:.../gc-session-t3` alias map to `t3`; every other selection maps to the tmux carrier. Provider construction and persisted session transport metadata reuse this mapping. | `internal/runtime/transport_name.go`; `internal/runtime/transport_name_test.go`; `cmd/gc/transport_axis_test.go`; `cmd/gc/session_transport_authoritative_test.go` |
