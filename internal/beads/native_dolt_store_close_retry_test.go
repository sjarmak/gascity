...
// error, which lets a test model a concurrent writer closing the bead during
// the retry backoff. That case is why the short-circuit matters. The retry
// sleeps between attempts, and another actor can act in that gap.
//
// Note what these tests deliberately do NOT model: a conflict reported after
// this store's own write committed. That cannot happen on this path.
// beadslib gives CloseIssue one commit boundary on both backends, and its
// withRetryTx comment records that 1213/1205 guarantee a server-side rollback.
// The two-commit-point window belongs to RunInTransaction, which Update and
// SetMetadataBatch use and Close does not.
...
// returns, so a test can model "a concurrent actor closed it during the
// backoff" as easily as "this attempt lost the race and nothing landed".
...
// the race, and while it backs off, a different writer closes the bead. The
...
func TestNativeDoltStoreCloseReplayShortCircuitsWhenAnotherActorClosedTheBead(t *testing.T) {
	spy := newCloseSpy(func(attempt int32, markClosed func()) error {
		if attempt == 1 {
			...
			// actor closed the bead during the backoff.
			markClosed()
			return errors.New(serializationConflictErr)
		}
