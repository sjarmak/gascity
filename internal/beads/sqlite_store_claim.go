...
func (s *SQLiteStore) claimTx(id, assignee string, mintGeneration bool) (Bead, string, bool, error) {
	...
	err := retryOnBusy(func() error {
		...
		if cur == assignee && b.Status == "in_progress" {
			...
			if mintGeneration {
				generation = b.Metadata[beadmeta.ClaimGenerationMetadataKey]
				if _, err := parseStoredClaimGeneration(generation); err != nil {
		...
		var next string
		if mintGeneration {
			next, err = NextClaimGeneration(b.Metadata[beadmeta.ClaimGenerationMetadataKey])
			if err != nil {
				return fmt.Errorf("claiming bead %q: %w", id, err)
			}
