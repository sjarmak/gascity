...
func TestNativeDoltStoreDeclaresConditionalWriterAndMetadataCAS(t *testing.T) {
...
func TestNativeDoltMemoryFixtureFailsUnsupportedGuardedRolesWithoutPanicking(t *testing.T) {
	store := newNativeDoltStoreForTest(newNativeDoltMemStorage())
	title := "updated"
	if err := store.UpdateIfMatch("gc-missing", 1, UpdateOpts{Title: &title}); err == nil {
		t.Fatal("UpdateIfMatch on limited memory fixture returned nil")
	}
	if err := store.CloseIfMatch("gc-missing", 1); err == nil {
		t.Fatal("CloseIfMatch on limited memory fixture returned nil")
	}
	if err := store.DeleteIfMatch("gc-missing", 1); err == nil {
		t.Fatal("DeleteIfMatch on limited memory fixture returned nil")
	}
}

func TestNativeDoltStoreConditionalWritesResolve(t *testing.T) {
	for _, mode := range []gate.Mode{gate.Auto, gate.Require} {
		t.Run(string(mode), func(t *testing.T) {
			...
			if err != nil || writer == nil || diag != nil {
...
func TestCachingStoreOverNativeDoltStoreForwardsConditionalWrites(t *testing.T) {
	...
	if !ok {
		t.Fatal("CachingStore over native backing does not resolve a MetadataCASWriter")
	}
	...
	if capable, _ := cache.probeConditionalWriteCapability(); !capable {
		t.Fatal("CachingStore does not report the native backing's conditional-write capability")
	}
}
