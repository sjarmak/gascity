...
func (c *CachingStore) Ready(query ...ReadyQuery) ([]Bead, error) {
	...
	var result []Bead
	for _, b := range openBeads {
		if cachedBeadReady(b, blockersByID, depsByID[b.ID]) {
...
func cachedBeadReady(b Bead, blockersByID map[string]blockerState, deps []Dep) bool {
	for _, dep := range deps {
		if !isReadyBlockingDependencyType(dep.Type) {
			continue
		}
		blocker, ok := blockersByID[dep.DependsOnID]
		if !ok {
			continue
		}
		// A terminally failed blocker never satisfies its dependents, even when
		// bd's is_blocked projection reports this bead unblocked: that
		// projection is computed from status alone and cannot tell a step that
		// finished from one that hard-failed (gc-d58o).
		if blocker.terminalFailure {
			return false
		}
		if b.IsBlocked == nil && blocker.status != "closed" {
			return false
		}
	}
	if b.IsBlocked != nil {
