...
// active log's tail and then archives newest-first until it finds one.
//
...
// via an unbounded read. events.ReadFiltered walks archives oldest-first and
// cannot express "the newest", so asking it for one controller start gunzipped
// every archive on the city — 37 files / 2.16 GB / 2.9M lines as measured on
// ds-research, where gzip decompression alone costs twice this check's timeout.
