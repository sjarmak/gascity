...
| SESSION-WORK-005 | Agent drain-ack provenance survives runtime death | `gc runtime drain-ack` persists agent provenance and a per-ack token through the session command boundary before publishing the runtime ack flag. The hook-triggered path binds backend opening and operations to the hook mutation deadline; runtime-only sessions without a durable bead retain provider-only acknowledgment behavior even when their task store is unavailable. Partial runtime publication preserves the durable marker; cancellation writes both a compatibility tombstone and a hashed per-token marker supported by every task-store backend, so a delayed clear cannot erase a newer cancellation. The next acknowledgement and terminal finalization remove obsolete per-token markers. Failed writes retry synchronously and leave a provider-backed retry request for the controller; superseded clears restore the runtime signal only for a newer active acknowledgement, including after a partial runtime-clear failure. Terminal finalization consumes the active provenance fields. Finalization uses an authoritative typed read, so an always-named session receives its bounded cooldown even when provider metadata disappears, the controller cache is stale, or its tick snapshot predates acknowledgement. Pool-managed, on-demand, reconciler-sourced, canceled, and assigned-work drains remain excluded. | issue `gc-fg8u7`; `internal/session/manager_test.go`; `cmd/gc/cmd_runtime_drain_test.go`; `cmd/gc/session_reconciler_test.go`; `cmd/gc/session_wake_test.go` |
