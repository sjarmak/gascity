...
| SESSION-START-010 | Pending-create rollback ownership | Rollback must re-read and close under the same process-local session mutation lock as completion and incarnation allocation. Completed, closed, or changed generation/token snapshots cause no rollback writes or folds. Partial failed-create closes remain retryable. Completion rechecks its existing lease verdict under that lock and cannot overwrite rollback. This fence does not serialize out-of-process direct bead writes. | `cmd/gc/session_pending_create_fence_test.go`; `internal/session/store_start_fence_test.go`; `cmd/gc/session_lifecycle_parallel_test.go` |
| SESSION-START-011 | Advisory awake state during creation | A failed Start still rolls back when heal changed its state to awake but left the pending claim and identity current. Active, cleared-claim, closed, and superseded-identity protections remain. | `cmd/gc/session_pending_create_fence_test.go` (`TestFailedStartRollsBackAfterHealToAwake`, `TestRollbackPendingCreateRejectsStaleSnapshot`); issue #6104 |
| SESSION-START-012 | Stale final start commit cleanup | If rollback wins between async refresh and the final atomic commit, the provider-aware caller cleans up only the attempted runtime identity, outside the mutation lock. Foreign or newer runtimes survive; ordinary persistence errors do not trigger cleanup or a woke event. | `cmd/gc/session_pending_create_fence_test.go` (`TestAsyncStartFinalCommitStaleCleanup`) |
...
| SESSION-RECON-015 | Agent API session origin | Both configured-agent API projections persist `session_origin=ephemeral` at create time, including generated adhoc identities. Provider/manual and configured named sessions retain their own origins. API creation does not by itself confer controller pool ownership; controller reconciliation may subsequently reclassify an ephemeral, pool-marker-free bead on a multi-session template (see `isLegacyManualSessionBeadForAgent`; issue #6095 tracks making the ephemeral origin durable, and #5009 stays open on it). | issue #5009; issue #6095; `internal/api/handler_sessions_test.go` (`TestHumaHandleSessionCreateUsesACPTransportCommandForAgentTemplate`); `cmd/gc/cmd_hook_test.go` (`TestCmdHookPoolDemandOriginGate`); `cmd/gc/build_desired_state_test.go` (`TestBuildDesiredState_APICreatedAgentSessionBeadStaysDesired`) |
| SESSION-RECON-016 | Occupied singleton retry | Fresh canonical singleton creation defers before allocating a bead while its derived runtime name is running, even without an open holder bead. Repeated demand neither mints failed records nor adopts/stops the owner. Creation resumes when the runtime vacates; provider start ownership checks still fence races after the probe. | `cmd/gc/session_pending_create_fence_test.go` (`TestSingletonPendingCreateRetryPreservesOccupiedRuntimeWithoutBeadChurn`) |
...
| SESSION-RUNTIME-009 | Runtime uncertainty defers lifecycle mutation | A runtime observation that wraps `ErrRuntimeUnavailable` remains unknown through session and worker handles. Reconciliation must not derive absence, detachment, or inactivity from that result: pending creation and wake-failure budgets remain intact; close/heal, pool sweep, drain completion, detached-at, idle/no-wake drain, and config-drift restart defer until a healthy observation. Confirmed empty observations retain the ordinary absence/convergence path, and non-sentinel legacy activity errors remain best-effort. | `internal/session/manager_test.go` `TestObserveRuntimeForInfoPreservesObservationUncertainty`; `internal/worker/handle_test.go` `TestRuntimeHandleLiveObservationPreservesObservationUncertainty`; `cmd/gc/session_lifecycle_start_boundary_test.go`; `cmd/gc/city_runtime_test.go`; `cmd/gc/session_wake_test.go`; `cmd/gc/session_observation_uncertainty_test.go`; `cmd/gc/session_reconciler_trace_test.go` `TestReconcileOrphanCloseFailsClosedOnLivenessError` |
