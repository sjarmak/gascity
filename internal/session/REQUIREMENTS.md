...
| SESSION-ID-012 | Controller-issued mail activation fence | A durable-mail attempt may derive authority only from one active persisted named session whose stable seat, positive generation, positive continuation epoch, instance token, city/config digest, and controller issuer are exact. Closed, draining, foreign-seat, incomplete, or caller-shaped identities are refused. Re-resolving unchanged logical authority retains the same content-addressed fence identity even when observation time changes. | `internal/session/mail_activation_fence.go`; `internal/session/mail_activation_fence_test.go`; `gc-dyfe8` |
