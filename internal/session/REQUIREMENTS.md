...
| SESSION-RUNTIME-007 | Authoritative persisted transport | New session beads record their resolved effective transport (`acp`, `tmux`, or the T3 runtime's `t3`) instead of leaving the default path unstamped. Reconciliation updates the stamp when effective transport changes and clears ACP-only MCP identity/snapshot metadata for every non-ACP transport. A non-empty stamp outranks legacy ACP heuristics; genuinely unstamped legacy beads retain the ACP-artifact fallback. | `internal/session/transport_metadata.go`; `internal/session/transport_authoritative_test.go`; `cmd/gc/session_transport_authoritative_test.go`; `internal/api/session_transport_authoritative_test.go`; `internal/worker/factory_test.go` |
