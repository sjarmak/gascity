...
| SESSION-WORK-005 | Agent drain-ack provenance survives runtime death | `gc runtime drain-ack` atomically persists agent provenance and a per-ack token through the session command boundary before publishing the runtime ack flag. Partial runtime publication preserves the durable marker; token-fenced cancellation cannot erase a newer acknowledgement, failed clears retry, and terminal finalization consumes both fields. Finalization uses an authoritative typed read, so an always-named session receives its bounded cooldown even when provider metadata disappears, the controller cache is stale, or its tick snapshot predates acknowledgement. Pool-managed, on-demand, reconciler-sourced, and assigned-work drains remain excluded. | issue `gc-fg8u7`; `internal/session/manager_test.go`; `cmd/gc/cmd_runtime_drain_test.go`; `cmd/gc/session_reconciler_test.go`; `cmd/gc/session_wake_test.go` |
