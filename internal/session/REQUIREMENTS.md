...
| SESSION-WORK-005 | Agent drain-ack provenance survives runtime death | `gc runtime drain-ack` persists agent provenance through the session command boundary before publishing the runtime ack flag. Finalization reads the typed durable field, so an always-named session receives its bounded cooldown even when provider metadata disappears before the first reconcile observation. Pool-managed, on-demand, reconciler-sourced, and assigned-work drains remain excluded. | issue `gc-fg8u7`; `internal/session/manager_test.go`; `cmd/gc/cmd_runtime_drain_test.go`; `cmd/gc/session_reconciler_test.go` |
