...
func TestCheckTriggerConditionFails(t *testing.T) {
	for _, check := range []string{"exit 1", "printf ' \\n\\t' >&2; exit 1"} {
		...
		if result.Reason != "check command failed: exit status 1" {
			t.Errorf("Reason = %q, want byte-identical silent failure reason", result.Reason)
		}
		if result.TimedOut {
...
func TestCheckTriggerConditionFailureOverflowStartsAtCompleteLine(t *testing.T) {
	// 35-byte lines, then LAST-LINE, then 3900 trailing spaces. The kept
	// 4096 bytes begin 11 bytes before the end of line 195, so the excerpt
	// must start at line 196, and it stays under the rune cap.
