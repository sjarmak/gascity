...
type TriggerResult struct {
	...
	Reason string
	// TimedOut is true only when a condition check exceeded its deadline.
	TimedOut bool
...
// check_timeout deadline. It remains stable for operator-facing text and
// callers that inspect the reason.
...
func checkCondition(a Order, opts TriggerOptions) TriggerResult {
	...
	if err := cmd.Run(); err != nil {
		...
		if excerpt := conditionCheckStderrExcerpt(stderrTail.String(), stderrTail.dropped, cmd.Env); excerpt != "" {
