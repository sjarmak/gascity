...
//
// Runs inside a synctest bubble so the write cadence and the idle timer both
// advance on the fake clock: virtual time only moves once every goroutine is
// durably blocked, so a slow scheduler under real CPU load (the cause of the
// original flake, gc-8ytgh) can never let more than 50ms of apparent silence
// elapse between writes.
